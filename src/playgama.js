...
(function (global) {
  ...
  function _patchCG() {
      ...
      return new Promise(function (resolve) {
        ...
        function onState(state) {
          ...
          if (state === 'closed' || state === 'failed') {
            ...
            // mock/qa_tool has no real ad to grant: 'failed' is the mock's
            // terminal state, so treat it as success — otherwise the reward
            // flow (daily retry, etc.) is untestable outside a live network.
            if (state === 'failed') { sawFailed = true; if (isMockR) granted = true; }
            finish();
          }
        }
        ...
        timeout = setTimeout(function () {
          sawFailed = true;
          if (isMockR) granted = true;   // mock may never emit a terminal state
          try { _bridge.advertisement.off(evRew, onState); } catch (e) {}
          finish();
        }, 15000);
    ...
    // Wrap interstitial + rewarded to auto-hide the banner before the ad
    // and restore it afterward — prevents banner overlapping the ad panel.
    var _origInter   = SG.CG.requestMidgameAd;
    var _origRewarded = SG.CG.requestRewardedAd;
    SG.CG.requestMidgameAd = async function () {
      try { await SG.CG.hideBanner(); } catch (e) {}
      try { return await _origInter.apply(this, arguments); } finally {
        try { SG.CG.showBanner(); } catch (e) {}
      }
    };
    SG.CG.requestMidgameAd.__SG_PG_PATCHED__  = true;
    SG.CG.requestMidgameAd.__SG_PG_PLATFORM__ = _pid;
    SG.CG.requestRewardedAd = async function () {
      try { await SG.CG.hideBanner(); } catch (e) {}
      try { return await _origRewarded.apply(this, arguments); } finally {
        try { SG.CG.showBanner(); } catch (e) {}
      }
    };
